...
Top-level fields are `schema_version`, `dataset_kind`, `conditions`, `splits`, and `outfits`. Official data uses `dataset_kind=official_12x200`; Gate 4 conversion uses `regression` and cannot satisfy official evidence claims. Optional `supervision_mode` defaults to `single_target`, preserving every existing manifest and loader behavior.
...
When `supervision_mode=dual_target_region_aware_v1`, training replaces the single `target_rgb` with `target_edit_rgb`, `target_base_rgb`, `target_edit_mask`, `target_edit_core_mask`, `target_preserve_mask`, `target_transition_mask`, `target_protected_mask`, `target_base_foreground_mask`, `target_old_clothing_mask`, and `target_revealed_skin_mask`. The existing `target_foreground_mask` is edited alpha (`A_edit`), `target_base_foreground_mask` is frozen-base alpha (`A_base`), and `target_clothing_mask` is the new clothing target with protected pixels removed. All dual-target values are supervision-only. `select_forward_conditioning_fields()` admits references plus target pose/camera only.

Inference returns the same references plus target pose/R/Th/K/w2c/camera. It rejects target RGB/masks, every dual-target field, teacher/anchor targets and any cloth/clothing embedding. `outfit_id` is metadata/routing only and is forbidden as model conditioning.
