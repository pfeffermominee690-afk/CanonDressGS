...
When `supervision_mode=dual_target_region_aware_v1`, training replaces the single `target_rgb` with `target_edit_rgb`, `target_base_rgb`, `target_edit_mask`, `target_edit_core_mask`, `target_preserve_mask`, `target_transition_mask`, `target_protected_mask`, `target_old_clothing_mask`, and `target_revealed_skin_mask`. The existing `target_foreground_mask` and `target_clothing_mask` are the edited foreground and new clothing targets. All dual-target values are supervision-only. `select_forward_conditioning_fields()` admits references plus target pose/camera only.
