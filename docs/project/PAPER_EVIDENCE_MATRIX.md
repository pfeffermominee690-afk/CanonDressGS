...
## 2026-07-18 R3-CLEAN GEOMCAM evidence

| Paper claim | Status | Code / Run | Current evidence | Limitation / permitted wording |
|---|---|---|---|---|
| The condition clay source is a clothed template or the current Gaussian base | REFUTED | `SUBJECT02-R3-CLEAN-GEOMCAM-002/attempt_002` / `93e79d6` | Saved dataset metadata, per-condition pose archives, camera JSON and clean clay renders identify a clean subject02 SMPL-X source without hair/shoes/clothing | The original generator source and exact historical SMPL-X binary fingerprint were not recovered |
| The recovered source replay closes the frozen camera/metadata Gate A | NOT SUPPORTED | same run | IoU min/mean/median `0.928023/0.975762/0.989554`; five conditions below 0.98; exact bbox 2/12, despite identity orientation and low center residual | Report as GC5 insufficient evidence; do not lower the frozen threshold or call it an exact reproduction |
| R3-CLEAN-001 proves the subject02 clean-body shape itself is wrong | NOT ADJUDICABLE | same run plus sealed R3-CLEAN-001 | R3-CLEAN-001 used a different flat-hand/big-pose/formal-LBS composition and target-mask bbox fitting; Gate A still fails under no-fit replay | Clean-body Gate B was deliberately not evaluated; neither shape failure nor shape PASS may be claimed |

## 2026-07-18 R3 O00 arm-support closure evidence

| Claim | Status | Evidence | Measurement | Paper wording constraint |
|---|---|---|---|---|
| A subject02-only six-region 12k frozen arm support can be built with formal SMPL-X barycentric 55-joint LBS | SUPPORTED | `SUBJECT02-O00-ARM-SUPPORT-CLOSURE-001/attempt_003` / `5bc0726` | 63,445 observed subject02 pixels; parts 16–19 only; support/base fingerprints exact; four views finite | This is a diagnostic O00 arm asset, not a validated full-body support representation |
| The current 12k arm support satisfies covered-state occlusion and revealed-arm repair | REFUTED | same run | best frozen candidate repair min/mean `0.782226/0.873649`, leakage max/mean `0.217774/0.126351`, covered max `0.035961`; all miss frozen thresholds | Report the occlusion–repair tradeoff; do not call the support production-ready |
| The fixed-open Gaussian Oracle proves or refutes O00 residual capacity after support | NOT TESTED | same run | hard `ARM_SUPPORT_PASS` prerequisite failed; Oracle optimizer steps `0`, no Oracle directory | Do not report edit/clothing reduction, short-sleeve formation, or capacity failure from this run |
| A formal full-body support base or formal image-conditioned training is now justified | UNSUPPORTED | same run | Case C safe stop; support representation remains invalid | A new separately authorized representation improvement is required before either downstream stage |
| The source-replay discrepancy is caused by hair, shoes or garment volume | REFUTED-FOR-AUDITED-SOURCE | same run | Source clay and masks contain only the clean parametric body; three opened panels show no hair/shoes/clothing shell | This does not characterize the separate 200k G_base outer shell |
| A nested-body Geometry Gate V2 has been validated | PROPOSED-NOT-VALIDATED | same run | Two-stage Gate A transform contract plus Gate B body-core outside fraction <=0.03 is recorded | Gate B was not reached; the proposal must not be called a formal or passed gate |
