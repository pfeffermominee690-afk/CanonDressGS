...
class AnchorClothingMLP(nn.Module):
    ...
    def forward_film(
        ...
        fused_anchor_features = self._fuse_local_features(
            anchor_features, anchor_clothing_features
        )

        hidden = self._anchor_input_linear(fused_anchor_features)
        hidden = self.activation((1 + film_gamma[0]) * hidden + film_beta[0])
        for layer_index, layer in enumerate(self.hidden_layers[1:], start=1):
            hidden = layer(hidden)
            hidden = self.activation(
                (1 + film_gamma[layer_index]) * hidden + film_beta[layer_index]
            )
        return self._split_output(self.output_layer(hidden))
