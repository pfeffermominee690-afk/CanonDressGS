...
def main():
 ...
 def predict():
  ...
  raw,bounded,gated=model.dressable_model.compute_film_anchor_residuals(bundle,clothing_embedding=encoded['global_clothing_embedding'],anchor_clothing_features=encoded['anchor_clothing_features'])
 ...
 with torch.no_grad(): encoded,raw,bounded,gated,gaussian,overrides,pred=predict()
 ...
 for name in CHANNELS:
  single=type(gaussian)(**{key:(getattr(gaussian,key) if key==name else torch.zeros_like(getattr(gaussian,key))) for key in CHANNELS}); ov=model.dressable_model.compose_canonical_gaussian_overrides(single,[name]); degree=1 if name=='delta_shN' else 0; control=render(episode['target_pose'],episode['target_Rh'],episode['target_Th'],episode['target_camera'],None,degree); rr=render(episode['target_pose'],episode['target_Rh'],episode['target_Th'],episode['target_camera'],ov,degree); attribution[name]={"rgb":diff(control[0],rr[0]),"alpha":diff(control[1],rr[1])}; save_render_tensor(out/f"{DISPLAY[name]}_only.png",rr[0],3); panels.append(_as_chw_render(rr[0],3).cpu())
  if name=='delta_shN': save_render_tensor(out/'shN_degree1_zero_control.png',control[0],3)
