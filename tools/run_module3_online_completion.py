...
def main():
 ap=argparse.ArgumentParser(); ap.add_argument('--gate4-config',required=True); ap.add_argument('--module2-config',required=True); ap.add_argument('--module3-config',required=True); ap.add_argument('--module2-output',required=True); ap.add_argument('--output-dir',required=True); ap.add_argument('--device',default='cuda'); ap.add_argument('--visual-inspection-record'); args=ap.parse_args()
 ...
 for name,o in final_outputs.items():
  ...
  metrics[name]={"geometry":mask_metrics(o.geometry_gate,geometry_target,observed),"appearance":mask_metrics(o.appearance_gate,appearance_target,observed),"coverage_mean":float(observed.mean()),"covered_anchor_ratio":float((observed>0).float().mean())}
 ...
 request_path=out/'independent_inference_request.pt'; response_path=out/'independent_inference_output.pt'
 request={k:(v.cpu() if isinstance(v,torch.Tensor) else v) for k,v in subset_episode(episode,[0,1]).items() if k in ('reference_images','reference_cloth_masks','reference_foreground_masks','reference_poses','reference_Rh','reference_Th','reference_cameras','reference_valid_mask')}; request.update(target_pose=episode['target_pose'].cpu(),target_Rh=episode['target_Rh'].cpu(),target_Th=episode['target_Th'].cpu(),target_camera=episode['target_camera'],target_height=int(episode['target_rgb'].shape[-2]),target_width=int(episode['target_rgb'].shape[-1])); torch.save(request,request_path)
 command=[sys.executable,str(Path(__file__).with_name('infer_module3_online_completion.py')),'--gate4-config',args.gate4_config,'--module2-config',args.module2_config,'--checkpoint',str(ck),'--request',str(request_path),'--output',str(response_path),'--device',args.device]; completed=subprocess.run(command,capture_output=True,text=True)
 if completed.returncode: raise RuntimeError(f"independent inference failed: {completed.stderr}")
 independent=torch.load(response_path,map_location='cpu',weights_only=True); expected={k:v[0].detach().cpu() for k,v in fields.items()}; expected.update({f"anchor.{n}":getattr(before['gated_anchor_residuals'],n).detach().cpu() for n in CHANNELS}); expected.update({f"Gaussian.{n}":getattr(before['gaussian_residuals'],n).detach().cpu() for n in CHANNELS}); parity={k:diff(expected[k],independent[k]) for k in expected}; parity.update({"forbidden_fields_count":0,"teacher_read":False,"target_image_read":False,"target_mask_read":False,"cloth_id_used":False,"gate_source":"online_reference_mask_completion","independent_process":True,"command":command,"request_fields":sorted(request)})
 ...
 for i in range(2): save_image(episode['reference_images'][i].cpu().clamp(0,1),out/f'reference_{i+1}.png'); save_image(episode['reference_cloth_masks'][i].cpu().clamp(0,1),out/f'reference_clothing_mask_{i+1}.png')
 ...
 pass_engineering=gradient['completer']>0 and all(v>0 for k,v in gradient.items() if k not in ('base_grad_count','completer','frozen_image_backbone_grad_count')) and gradient['base_grad_count']==0 and gradient['frozen_image_backbone_grad_count']==0 and all(v['allclose'] for v in roundtrip.values()) and all(v['allclose'] for v in parity.values() if isinstance(v,dict) and 'allclose' in v) and parity['independent_process'] and repeat_allclose and base.sh_degree==base_degree
 s12=metrics['S12']; learned_un=s12['geometry']['unobserved'].get('recall',0); observed_un=baseline['observed_only']['geometry']['unobserved'].get('recall',0); coverage_monotonic=s12['covered_anchor_ratio']>=max(metrics['S1']['covered_anchor_ratio'],metrics['S2']['covered_anchor_ratio']); pass_learning=loss_drop>=.3 and s12['geometry']['all']['F1']>=.9 and s12['geometry']['all']['IoU']>=.82 and s12['appearance']['all']['F1']>=.9 and s12['appearance']['all']['IoU']>=.82 and learned_un>=.6 and learned_un-observed_un>=.15 and s12['geometry']['inactive_mean']<=.1 and s12['appearance']['inactive_mean']<=.1 and feature_metrics['relative_improvement']>=.2 and coverage_monotonic
 inspection=None if args.visual_inspection_record is None else json.loads(Path(args.visual_inspection_record).read_text(encoding='utf-8')); build_visual_acceptance('module3',out,out,inspection); visual_status=json.loads((out/'visual_acceptance.json').read_text(encoding='utf-8'))['visual_acceptance_status']
 passed=pass_engineering and pass_learning and render_metrics['learned']['clothing_rgb_l1']<render_metrics['observed_only']['clothing_rgb_l1'] and render_metrics['learned']['mask_iou']>=render_metrics['observed_only']['mask_iou'] and visual_status=='PASS'
 summary={"status":"PASS" if passed else "PARTIAL","loss_drop":loss_drop,"selected_step":best_step,"selection_rule":"minimum training total after min_steps; no acceptance metric used","engineering":pass_engineering,"learning":pass_learning,"visual_acceptance_status":visual_status,"coverage_monotonic":coverage_monotonic,"coverage_metric":"covered_anchor_ratio=(observation_coverage>0).mean; per-anchor coverage formula remains unchanged","coverage_formula_conflict":None,"metrics":metrics,"baseline":baseline,"feature":feature_metrics,"render":render_metrics,"gradient":gradient}
 (out/'forward.log').write_text(json.dumps(summary,indent=2)); (out/'GATE_ACCEPTANCE.md').write_text(f"# Gate 6 Online Completion\n\nStatus: **{'PASS' if passed else 'PARTIAL'}**\n\n- Engineering: {'PASS' if pass_engineering else 'FAIL'}\n- Learning thresholds: {'PASS' if pass_learning else 'FAIL'}\n- Coverage metric: covered-anchor union ratio; per-anchor formula unchanged\n- Independent inference: separate Python process, {'PASS' if parity['independent_process'] and all(v['allclose'] for v in parity.values() if isinstance(v,dict) and 'allclose' in v) else 'FAIL'}\n",encoding='utf-8'); print(json.dumps(summary,indent=2))
