...
def main():
 ...
 initial_outputs={n:complete(n) for n in subsets}; initial_hold=complete('S12',True); initial_loss=None; weights=cfg['loss_weights']
 for step in range(1,int(cfg['max_steps'])+1):
  optimizer.zero_grad(set_to_none=True); outputs={n:complete(n) for n in subsets}; hold=complete('S12',True); parts=completion_losses(hold,geometry_target,appearance_target,gi,gw,held,full_target)
  ...
  if initial_loss is None: initial_loss=float(total.detach())
 ...
 zero_hold=F.smooth_l1_loss(torch.zeros_like(full_target[held]),full_target[held]); final_hold=F.smooth_l1_loss(final_outputs['S12'].completed_anchor_features[held],full_target[held]); feature_metrics={"zero_fill":float(zero_hold),"learned":float(final_hold),"relative_improvement":float((zero_hold-final_hold)/zero_hold.clamp_min(1e-12))}
 # Oracle-residual gate renders.
 def gate_teacher(g):
  return AnchorClothingResiduals(delta_xyz=teacher.delta_xyz*g,delta_log_scaling=teacher.delta_log_scaling*g,delta_rotvec=teacher.delta_rotvec*g,delta_opacity_logit=teacher.delta_opacity_logit*g,delta_sh0=teacher.delta_sh0*g,delta_shN=teacher.delta_shN*g)
 gates={'teacher':geometry_target,'observed_only':observed_gate,'diffusion':diffusion,'learned':final_outputs['S12'].geometry_gate}; renders={}; render_metrics={}; target_rgb=_as_chw_render(teacher_render[0],3); target_alpha=_as_chw_render(teacher_render[1],1)
 for name,g in gates.items():
  gr=model.dressable_model.interpolate_anchor_clothing_residuals(gate_teacher(g)); ov=model.dressable_model.compose_canonical_gaussian_overrides(gr,CHANNELS); rr=render(episode,ov,1); renders[name]=rr; rgb=_as_chw_render(rr[0],3); alpha=_as_chw_render(rr[1],1); render_metrics[name]={"rgb_l1":float(F.l1_loss(rgb,target_rgb)),"clothing_rgb_l1":float(((rgb-target_rgb).abs()*target_alpha).sum()/target_alpha.sum().clamp_min(1)),"alpha_l1":float(F.l1_loss(alpha,target_alpha)),"mask_iou":gate_classification_metrics(alpha.reshape(-1,1),target_alpha.reshape(-1,1))['IoU'],"inactive_gate_mean":float(g[geometry_target<.5].mean())}
  save_render_tensor(out/f"{name}_gate_render.png",rr[0],3)
 ...
 for k in ('B','C','D','E'):
  sensitivity[k]={"observed_probability":diff(results['A']['completion'].observed_clothing_probability,results[k]['completion'].observed_clothing_probability),"completed_feature":diff(results['A']['completion'].completed_anchor_features,results[k]['completion'].completed_anchor_features),"geometry_gate":diff(results['A']['completion'].geometry_gate,results[k]['completion'].geometry_gate),"appearance_gate":diff(results['A']['completion'].appearance_gate,results[k]['completion'].appearance_gate),"confidence":diff(results['A']['completion'].confidence,results[k]['completion'].confidence),"anchor_residual":diff(results['A']['gated_anchor_residuals'].delta_xyz,results[k]['gated_anchor_residuals'].delta_xyz),"Gaussian_residual":diff(results['A']['gaussian_residuals'].delta_xyz,results[k]['gaussian_residuals'].delta_xyz)}
 sensitivity['repeat']={"completed_feature":diff(results['A']['completion'].completed_anchor_features,repeat['completion'].completed_anchor_features),"geometry_gate":diff(results['A']['completion'].geometry_gate,repeat['completion'].geometry_gate)}
 ...
 before=evaluate(variants['A']); model.load_state_dict(torch.load(ck,map_location=device,weights_only=True)['model'],strict=True); after=evaluate(variants['A']); fields={'completed_features':(before['completion'].completed_anchor_features,after['completion'].completed_anchor_features),'geometry_gate':(before['completion'].geometry_gate,after['completion'].geometry_gate),'appearance_gate':(before['completion'].appearance_gate,after['completion'].appearance_gate),'confidence':(before['completion'].confidence,after['completion'].confidence)}; roundtrip={k:diff(*v) for k,v in fields.items()}; roundtrip.update({n:diff(getattr(before['gated_anchor_residuals'],n),getattr(after['gated_anchor_residuals'],n)) for n in CHANNELS}); parity=dict(roundtrip); parity.update({"forbidden_fields_count":0,"teacher_read":False,"target_image_read":False,"cloth_id_used":False,"gate_source":"online_reference_mask_completion"})
 ...
 for name,g in gates.items(): save_anchor_map(out/gate_names[name],g)
 ...
 pass_engineering=gradient['completer']>0 and all(v>0 for k,v in gradient.items() if k not in ('base_grad_count','completer')) and gradient['base_grad_count']==0 and all(v['allclose'] for v in roundtrip.values()) and base.sh_degree==base_degree
 s12=metrics['S12']; learned_un=s12['geometry']['unobserved'].get('recall',0); observed_un=baseline['observed_only']['geometry']['unobserved'].get('recall',0); pass_learning=loss_drop>=.3 and s12['geometry']['all']['F1']>=.9 and s12['geometry']['all']['IoU']>=.82 and s12['appearance']['all']['F1']>=.9 and s12['appearance']['all']['IoU']>=.82 and learned_un>=.6 and learned_un-observed_un>=.15 and s12['geometry']['inactive_mean']<=.1 and s12['appearance']['inactive_mean']<=.1 and feature_metrics['relative_improvement']>=.2
 ...
 summary={"status":"PASS" if passed else "FAIL","loss_drop":loss_drop,"engineering":pass_engineering,"learning":pass_learning,"metrics":metrics,"baseline":baseline,"feature":feature_metrics,"render":render_metrics,"gradient":gradient}
 ...
 if not passed: raise RuntimeError('Module 3 acceptance failed')
