"""Real-renderer no-training gate for the dual-support micro-pilot."""
...
def main() -> None:
    ...
    manifest = runner.read_json(runner.FROZEN_MANIFEST)
    ...
    with previous.NoTrainingProvenance(attempt, "dual_support_renderer_gate") as provenance:
        runtime_value = previous.runtime(attempt, asset_root, runner.protocol())
        provenance.capture_frozen_runtime_baseline()
        endpoints = runner.endpoint_residuals(runtime_value)
        hard = runner.hard_geometry_residual(endpoints["O01"], endpoints["O02"], endpoints["O01"], 0.5)
        with torch.inference_mode():
            hard_rgb, hard_alpha, hard_diag = runner.render_branches(
                runtime_value, "O01", "cond_000000", (("O01", hard, 1.0),)
            )
            dual_rgb, dual_alpha, dual_diag = runner.render_branches(
                runtime_value,
                "O01",
                "cond_000000",
                (("O01", endpoints["O01"], 0.5), ("O02", endpoints["O02"], 0.5)),
            )
    provenance_record = previous.aggregate_optimizer_provenance(attempt)
    ...
    if not all(checks.values()):
        raise RuntimeError(f"MICRO-PILOT-NO-TRAINING-GATE-VIOLATION: {checks}")
    report = {
        "schema_version": "canondressgs.research.geometry_dual_support_no_training_gate.v1",
        "status": "PASS",
        ...
        "frozen_assets_before": assets_before,
        "frozen_assets_after": assets_after,
        "frozen_trees_before": trees_before,
        "frozen_trees_after": trees_after,
        "archives_before": archives_before,
        "archives_after": archives_after,
        "paper_final": False,
    }
    runner.atomic_json(attempt / "audits/no_training_gate_test_report.json", report)
    print(json.dumps({"status": "PASS", "test_count": len(checks), "hard_shape": list(hard_rgb.shape), "dual_shape": list(dual_rgb.shape)}, sort_keys=True))
